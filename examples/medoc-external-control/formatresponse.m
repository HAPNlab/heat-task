...
% FORMATRESPONSE decode the response from byte array (message) to readable 
...
%% Segmentation points for decoding response
    ...
    if (responseLength + length(LENGTH_OFFSET)) ~= nBytes
        ...
        res.responseLength = sprintf('Response Length: %d Bytes',nBytes);%responseLength);
    end % end if
    ...
    res.pathwayState = ['System State: ',pathwayState];
    ...
    res.testTime = ['Test Time: ',msec2hms(testTime)]; 
     ...
     % Temp
    testTemp = decode(message(TEMP_OFFSET),8)/100;
    res.testTemp = ['Test Temp: ',num2str(testTemp)] ;
   
     % COVAS
    testCOVAS = decode(message(COVAS_OFFSET),8);
    res.testCOVAS = ['CoVAS: ',num2str(testCOVAS)]; 
    
     % YES
    testYES = decode(message(YES_OFFSET),8);
    res.testYES = ['YES: ',num2str(testYES)]; 
    
    
     % NO
    testNO = decode(message(NO_OFFSET),8);
    res.testNO = ['NO: ',num2str(testNO)]; 
    
     % TTL
     
    testTTL = message(TTL_OFFSET);%decode(message(TTL_OFFSET),8);
    res.testTTL = ['TTL: ',num2str(testTTL)]; 
